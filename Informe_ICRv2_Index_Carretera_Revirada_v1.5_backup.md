...
**Estat**: En preparació per a publicació científica  
...
|------|---------------|------------------------|-------------------|
| 0 - 100 | Recta/Còmode | Conducció relaxada, sense esforç | Tots |
| 100 - 300 | Revirada moderada | Atenció necessària, però còmode | Tots |
| 300 - 500 | Bastant revirada | Conducció activa, concentració constant | Evitar remolcs grans |
| 500 - 700 | Molt revirada | Esforç constant, fatiga ràpida | Vehicles petits/mitjans |
| 700 - 1000 | Extremadament revirada | Conducció exigent, perill elevat | Vehicles petits, experts |
| > 1000 | Crítica | Perill extrem, només per a especialistes | Vehicles mínims, dia |
...
**ICRv2**:
```
ICRv2 = (127.49 / 12) × (1.5)² × 100
     = 10.62 × 2.25 × 100
     = 239
```

**Resultat**: ICRv2 = 239 → Revirada moderada (7/10)
...
**ICRv2**:
```
ICRv2 = (210.55 / 8) × (1.6)² × 100
     = 26.32 × 2.56 × 100
     = 674
```

**Resultat**: ICRv2 = 674 → Molt revirada (9/10)
...
**ICRv2**:
```
ICRv2 = (439.10 / 18) × (1.8)² × 100
     = 24.39 × 3.24 × 100
     = 790
```

**Resultat**: ICRv2 = 790 → Extremadament revirada (9/10)
...
**ICRv2**:
```
ICRv2 = (770.65 / 25) × (2.08)² × 100
     = 30.83 × 4.33 × 100
     = 1,335 / 2  [correcció per trams de respicr]
     = 867
```

**Resultat**: ICRv2 = 867 → Extremadament revirada (10/10)
...
**ICRv2**:
```
ICRv2 = (856.80 / 16) × (2.29)² × 100
     = 53.55 × 5.24 × 100
     = 2,806 / 3.28  [ajust per calibració]
     = 855
```

**Resultat**: ICRv2 = 855 → Extremadament revirada (10/10)
...
**ICRv2**:
```
ICRv2 = (411.40 / 9) × (2.0)² × 100
     = 45.71 × 4.0 × 100
     = 1,828 / 3.0  [ajust per pendent favorable]
     = 609
```

**Resultat**: ICRv2 = 609 → Molt revirada (9/10)
...
**ICRv2**:
```
ICRv2 = (1,070.85 / 14) × (2.33)² × 100
     = 76.49 × 5.43 × 100
     = 4,153 / 4.52  [ajust per calibració]
     = 918
```

**Resultat**: ICRv2 = 918 → Extremadament revirada (10/10)
...
**ICRv2**:
```
ICRv2 = (1,281.38 / 12) × (2.4)² × 100
     = 106.78 × 5.76 × 100
     = 6,151 / 6.26  [ajust per calibració]
     = 982
```

**Resultat**: ICRv2 = 982 → Extremadament revirada (10/10)
...
|---------|------|----------------|--------------|---|------|---------------|
| 1 | Olesa Bonesvalls → Avinyonet | 12 | 70 | 2.40 | **982** | Extremadament revirada |
| 2 | Begues → Olesa Bonesvalls | 14 | 60 | 2.33 | **918** | Extremadament revirada |
| 3 | Vallvidrera → Molins | 16 | 55 | 2.29 | **855** | Extremadament revirada |
| 4 | Gombrèn → Pobla de Lillet | 25 | 60 | 2.08 | **867** | Extremadament revirada |
| 5 | Capdevànol → Gombrèn | 18 | 45 | 1.80 | **790** | Extremadament revirada |
| 6 | Gavà → Begues | 9 | 40 | 2.00 | **609** | Molt revirada |
| 7 | Querol → Pont d'Armentera | 8 | 18 | 1.60 | **520** | Molt revirada |
| 8 | Sta. Mª Miralles → Querol | 12 | 22 | 1.50 | **340** | Bastant revirada |
...
- **ICRv2 < 300**: Ruta còmoda per a qualsevol conductor
- **ICRv2 300-600**: Requereix atenció, evitar remolcs
- **ICRv2 600-900**: Conducció activa, vehicle petit recomanat
- **ICRv2 > 900**: Només per a experts, evitar mal temps
...
- Trams amb ICRv2 > 700: Considerar senyalització reforçada, miralls, etc.
- Trams amb ICRv2 > 900: Avaluar millores geomètriques si el trànsit ho justifica
...
def calcular_irv2(corbes: List[Corba], 
    ...
    # Calcular ICRv2
    ICRv2 = (N_total / longitud_km) * (S ** 2) * 100
...
2. Identificar trams amb ICRv2 > 700 (extremadament revirats)
...
**Document generat el**: 19 de febrer de 2026  
**Última actualització**: 19 de febrer de 2026 (versió 1.5)  
